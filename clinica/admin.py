...
class CustomUserAdmin(UserAdmin):
    ...
    add_fieldsets = (
        (None, {
            "classes": ("wide",),
            "fields": ("email", "nome", "sobrenome", "cpf", "user_type", "password1", "password2", "is_active", "is_staff", "is_superuser"),
        }),
    )
...
class ConsultaAdmin(admin.ModelAdmin):
    list_display = ('pet', 'veterinario', 'get_data_hora', 'status', 'status_pagamento') # NOVO CAMPO
...
class GerenciamentoPagamentoAdmin(admin.ModelAdmin):
    list_display = ('consulta_link', 'comprovante_link', 'atendente', 'status', 'data_atualizacao')
