...
def home_atendente(request):
    ...
    consultas_do_dia_count = Consulta.objects.filter(
        horario_agendado__data=hoje, status__in=["EM_ANDAMENTO", "CONFIRMADO"]
    ).count()
    consultas_pendentes_count = Consulta.objects.filter(
        status__in=["EM_ANDAMENTO", "CONFIRMADO"], horario_agendado__data__gte=hoje
    ).count()

    consultas_cadastradas = Consulta.objects.filter(
        horario_agendado__data__gte=timezone.now()
    )
...
def reservar_horario(request, horario_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            f"Horário reservado para {cliente.user.get_full_name()} e o pet {pet.nome}!",
        )
        return redirect("gerenciar_horarios")
